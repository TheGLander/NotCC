...
typedef struct Region {
  ...
  size_t _at_the_end_of_this_struct_size;  // do not dereference, this is
                                           // present for convenient struct
                                           // sizeof() computation only
} Region;
...
static void dump_memory_regions() {
  ...
  while (root) {
    ...
    MAIN_THREAD_ASYNC_EM_ASM(
        out('Region block ' + ptrToString($0) + ' - ' + ptrToString($1) + ' (' +
            toString(Number($2)) + ' bytes):'),
        r, lastRegionEnd, lastRegionEnd - (uint8_t*)r);
    while ((uint8_t*)r < lastRegionEnd) {
      MAIN_THREAD_ASYNC_EM_ASM(
          out('Region ' + ptrToString($0) + ', size: ' + toString(Number($1)) +
              ' (' + ($2 ? "used" : "--FREE--") + ')'),
...
#ifdef EMMALLOC_VERBOSE
  MAIN_THREAD_ASYNC_EM_ASM(
      out('claim_more_memory: claimed ' + ptrToString($0) + ' - ' +
          ptrToString($1) + ' (' + Number($2) + ' bytes) via sbrk()'),
...
static void* attempt_allocate(Region* freeRegion,
                              size_t alignment,
                              size_t size) {
  ASSERT_MALLOC_IS_ACQUIRED();
...
#ifdef EMMALLOC_VERBOSE
  MAIN_THREAD_ASYNC_EM_ASM(
      out('attempt_allocate(freeRegion=' + ptrToString($0) + ',alignment=' +
          Number($1) + ',size=' + Number($2) + ')'),
...
#ifdef EMMALLOC_VERBOSE
  MAIN_THREAD_ASYNC_EM_ASM(
      out('attempt_region_resize(region=' + ptrToString($0) + ', size=' +
          toString(Number($1)) + ' bytes)'),
      region, size);
#endif
...
#ifdef EMMALLOC_VERBOSE
  MAIN_THREAD_ASYNC_EM_ASM(
      out('aligned_realloc(ptr=' + ptrToString($0) + ', alignment=' + $1 +
...
void* emmalloc_aligned_realloc_uninitialized(void* ptr,
                                             size_t alignment,
                                             size_t size) {
  if (!ptr) {
    return emmalloc_memalign(alignment, size);
...
//   // let's define this as the difference between heap size and dynamic top
//   end. info.arena = emscripten_get_heap_size() - (size_t)sbrk(0);
...
//   info.hblkhd = 0; // Amount of bytes in mmapped regions: always 0. (no mmap
//   support)
//
...
//   malloc_trim(3). Region *lastActualRegion =
//   prev_region((Region*)(listOfAllRegions->endPtr - sizeof(Region)));
//   info.keepcost = region_is_free(lastActualRegion) ? lastActualRegion->size :
//   0;
//
//   RootRegion *root = listOfAllRegions;
//   while (root) {
...
//         info.fordblks += region_payload_end_ptr(r) -
