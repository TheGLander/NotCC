    ...
    for p in self.prefixes:
      if name.startswith(p['prefix']):
        if p['replacement'] is None:
          result = removeprefix(name, p['prefix'])
        else:
          result = p['replacement'] + removeprefix(name, p['prefix'])
        break
...
def strip_debug_sections(wasm):
  ...
  while pos < len(wasm):
    ...
    if section_id == 0:
      ...
      name = wasm[name_pos:name_end].decode("utf-8")
...
def append_source_mapping(wasm, url):
  ...
  section_content = encode_uint_var(len(section_name)) + section_name.encode("utf-8") + encode_uint_var(len(url)) + url.encode("utf-8")
