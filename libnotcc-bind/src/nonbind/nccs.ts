...
export function writeNCCS(saveData: ISetInfo): Uint8Array {
	...
	// Write the length of the version string before the string (including the null character)
	headerView.setUint32(NCCS_VERSION.length + 1, 4, true)
...
const MAGIC_STRING_AS_U32 =
	MAGIC_STRING.charCodeAt(0) * 0x1 +
	MAGIC_STRING.charCodeAt(1) * 0x100 +
	MAGIC_STRING.charCodeAt(2) * 0x10000 +
	MAGIC_STRING.charCodeAt(3) * 0x1000000

export function parseNCCS(data: ArrayBuffer): ISetInfo {
	const view = new DataView(data)
	const magicText = view.getUint32(0, true)
	if (magicText !== MAGIC_STRING_AS_U32) throw new Error("Missing magic string")

	const versionLength = view.getUint32(4, true)
